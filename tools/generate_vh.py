...
for i,l in enumerate(layers):
    ...
    if l["type"]=="conv": 
        ...
        text.append(f"`define L{idx}_BL_REQ_LEN             { l['filter']}")
        ...
        text.append(f"`define L{idx}_BL_BANK_DEPTH          {  math.ceil(l['filter'] / max_group_filter)}")
        text.append(f"`define L{idx}_WL_BANK_DEPTH          {l['kernel'] * l['kernel'] * l['channel']}")
        text.append(f"`define L{idx}_BL_FILT_GRP_STRIDE     {min(l['filter'],max_group_filter)}")
