...
module loader_addr_gen #(
    ...
    input      [            `CLOG2_SAFE(`DDR_DEPTH)-1:0] i_tl_src0_base_addr,
    input      [            `CLOG2_SAFE(`DDR_DEPTH)-1:0] i_tl_src1_base_addr,
    input      [`CLOG2_SAFE(`MAX_CHANNEL_GROUP_NUM) : 0] i_tl_ch_grp_split
);
  // ====================== parmeter =======================   
  integer                                           i;
  // ====================== reg ============================     
  ...
  always @(*) begin
    ...
    o_tl_addr = w_tl_base_addr + r_tl_col_offset + r_tl_row_offset + r_tl_ch_offset;
  end
  ...
  always @(posedge i_clk or negedge i_rstn) begin
    if (~i_rstn) begin
      ...
      if (i_nxt_lyr) begin
        ...
        // TODO : 더 나은 방법 개선 필요
        if (r_tl_ch_grp_idx != i_tl_ch_grp_split - 1) begin
