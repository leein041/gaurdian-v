...
module weight_req_gen #(
    parameter WBUF_DEPTH         = 0,
    ...
    //
    input                                                                          i_que_full,
    input      [                                `CLOG2_SAFE(MAX_WGT_BANK_DEPTH):0] i_bank_depth,
    input      [                                  `CLOG2_SAFE(MAX_WGT_BANK_NUM):0] i_bank_num,
    // 
    input      [                                      `CLOG2_SAFE(WBUF_DEPTH) : 0] i_req_len,
    input      [                                      `CLOG2_SAFE(WBUF_DEPTH)-1:0] i_req_addr,
    // request que
    output reg                                                                     o_req,
    output reg [                                      `CLOG2_SAFE(WBUF_DEPTH) : 0] o_req_len,
    output reg [                                      `CLOG2_SAFE(WBUF_DEPTH)-1:0] o_req_addr,
    // desc fifo
    output reg                                                                     o_desc_vld,
    output reg [`CLOG2_SAFE(MAX_WGT_BANK_DEPTH)+`CLOG2_SAFE(MAX_WGT_BANK_NUM)+1:0] o_desc_dout
);
  // ====================== parmeter =======================  
