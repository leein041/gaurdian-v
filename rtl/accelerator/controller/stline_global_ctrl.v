...
module stline_global_ctrl #(
    ...
    localparam IPT_ADDR  = $clog2(INPUT_DEPTH),
    localparam ACT_ADDR  = $clog2(IMAGE_DEPTH),
    localparam OPT_ADDR = $clog2(OUTPUT_DEPTH)
) (
    input                    i_clk,
    input                    i_rstn,
    input                    i_st,
  ...
  // ipt 
  reg                    r_ibuf_re;
  reg  [ IPT_ADDR-1:0] r_ibuf_rcnt;
  reg  [ IPT_ADDR-1:0] r_ibuf_addr;
  // opt  
  reg                    r_obuf_we;
  ...
  always @(posedge i_clk or negedge i_rstn) begin
    if (~i_rstn) begin
      ...
      r_ibuf_re   <= 'b0;
      r_ibuf_rcnt <= 'd0;
      r_ibuf_addr <= {IPT_ADDR{1'b1}};
      r_obuf_we   <= 'd0;
      r_obuf_addr <= {OPT_ADDR{1'b1}};
      r_obuf_wcnt <= 'd0;
      r_obuf_dat  <= 'd0;
      r_o_done    <= 'b0;
    end else begin
